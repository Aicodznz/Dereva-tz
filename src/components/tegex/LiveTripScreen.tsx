      ...
      <AnimatePresence>
        {showDetails && (
          <motion.div 
            ...
            className={`absolute bottom-0 left-0 right-0 w-full rounded-t-[28px] border-t p-4 pb-6 shadow-[0_-12px_30px_rgba(0,0,0,0.15)] z-[60] transition-all touch-none pointer-events-auto ${theme === 'dark' ? 'bg-[#111118]/95 border-neutral-800' : 'bg-white/95 border-neutral-200/80'}`}
          >
            {/* Header: Status and Drag Handle */}
            <div className="relative flex items-center justify-between mb-3 select-none">
              <div className="flex items-center gap-2">
                <div className={`border px-2.5 py-0.5 rounded-full flex items-center gap-1.5 animate-fade ${theme === 'dark' ? 'bg-emerald-950/20 text-emerald-400 border-emerald-900/60' : 'bg-emerald-50 text-emerald-700 border-emerald-500/20'}`}>
                  <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                  <span className="text-[8.5px] font-black uppercase tracking-wider leading-none">{statusText}</span>
                </div>

                {activeViewersCount > 0 && (
                  <div 
                    title={activeViewerNames.join(", ")}
                    className={`relative group border px-2.5 py-0.5 rounded-full flex items-center gap-1 shadow-sm cursor-help ${theme === 'dark' ? 'bg-blue-950/20 text-blue-400 border-blue-900/40' : 'bg-blue-50 text-blue-700 border-blue-400/20'}`}
                  >
              ...
              <div className={`w-10 h-1 rounded-full cursor-grab active:cursor-grabbing absolute left-1/2 -translate-x-1/2 ${theme === 'dark' ? 'bg-neutral-800' : 'bg-neutral-200'}`} />

              <button 
                onClick={() => setIsCollapsed(true)}
                className={`text-[9px] font-black uppercase tracking-[0.12em] px-2.5 py-0.5 rounded-full transition-colors pointer-events-auto ${theme === 'dark' ? 'text-neutral-400 bg-neutral-900 hover:bg-neutral-850 hover:text-neutral-200' : 'text-neutral-500 hover:text-neutral-800 bg-neutral-100 hover:bg-neutral-200'}`}
              >
                Ficha Maelezo
              </button>
            </div>
            ...
            {ride.driverInfo && (
              <div className={`flex items-center justify-between gap-3 p-3 rounded-xl border mb-3 select-none transition-all ${theme === 'dark' ? 'bg-[#161622]/60 border-neutral-800/80 hover:bg-neutral-800/20' : 'bg-neutral-50/80 border-neutral-200/50 hover:bg-neutral-100'}`}>
                {/* Left Side: Driver and Vehicle */}
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  <div className="w-9 h-9 rounded-full overflow-hidden border border-emerald-500 relative bg-neutral-100 shrink-0">
                    <img 
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <div className="flex items-center gap-0.5 text-yellow-500 shrink-0">
                        ...
                        <span className="text-[9px] font-black font-mono leading-none">{ride.driverInfo.rating || '4.8'}</span>
                      </div>
                    </div>
                    <p className={`text-[9px] font-semibold uppercase tracking-wider mt-1 truncate ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>
                ...
                <div className="text-right flex flex-col items-end shrink-0 max-w-[45%]">
                  <div className="flex items-center gap-1">
                    <span className={`text-[8px] font-black text-neutral-400 uppercase tracking-widest leading-none`}>{targetLabel}:</span>
                    <span className={`text-[9.5px] font-bold uppercase truncate max-w-[90px] leading-none ${theme === 'dark' ? 'text-neutral-200' : 'text-neutral-700'}`}>
                      {targetLocation.address}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className={`text-[7px] font-black uppercase tracking-wider px-1 py-0.5 rounded ${theme === 'dark' ? 'bg-indigo-950/40 text-indigo-400 border border-indigo-900/40' : 'bg-indigo-50 text-indigo-600 border border-indigo-100'}`}>
                      ETA
                    </span>
            ...
            <div className="mb-3 select-none pointer-events-auto">
              <div className={`relative h-10 rounded-xl border overflow-hidden p-2 flex flex-col justify-center ${theme === 'dark' ? 'bg-[#161622]/40 border-neutral-800/80 shadow-[0_4px_12px_rgba(0,0,0,0.1)]' : 'bg-neutral-50 border-neutral-200/60 shadow-sm'}`}>
                {/* Slim Road Line */}
                <div className={`w-full h-1 rounded-full relative overflow-visible flex items-center ${theme === 'dark' ? 'bg-neutral-800' : 'bg-neutral-200'}`}>
                  {/* Origin Point Marker */}
                  <div className="absolute left-0 -translate-x-1/2 -top-2 text-[10px] z-10">📍</div>
                  
                  {/* Destination Point Marker */}
                  <div className="absolute right-0 translate-x-1/2 -top-2 text-[10px] z-10">🏁</div>
                  ...
                  <motion.div 
                    className="absolute -top-4.5 h-6 w-6 z-20 flex items-center justify-center select-none"
                  ...
                  >
                    {ride.vehicleType === 'bike' ? (
                      <span className="text-sm" style={{ transform: 'scaleX(-1)' }}>🏍️</span>
                ...
                <div className="flex justify-between items-center mt-1.5 font-black text-[7.5px] uppercase tracking-wider text-neutral-400">
                  ...
                  <span className={`font-black font-mono px-1.5 py-0.2 rounded border text-[7px] ${theme === 'dark' ? 'bg-emerald-950/20 border-emerald-900/40 text-emerald-400' : 'bg-emerald-50 border-emerald-500/20 text-emerald-700'}`}>
                    {progress}% YA SAFARI
                  </span>
              ...
              <div className="flex justify-between items-center px-1 mt-1">
                {[
                  ...
                  <div key={s.label} className="flex flex-col items-center">
                    ...
                    <span className={`text-[5.5px] font-black uppercase tracking-widest mt-0.5 ${s.active ? (theme === 'dark' ? 'text-neutral-300' : 'text-neutral-800') : 'text-neutral-500'}`}>{s.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className={`grid grid-cols-4 gap-2 mt-3 pt-3 border-t ${theme === 'dark' ? 'border-neutral-800/60' : 'border-neutral-100'}`}>
              {/* Distance Left Panel */}
              <div className={`col-span-1 rounded-xl p-1.5 border flex flex-col items-center justify-center text-center select-none ${theme === 'dark' ? 'bg-[#161622]/40 border-neutral-800/80' : 'bg-neutral-50 border-neutral-200/60'}`}>
                <Navigation2 className="w-3.5 h-3.5 text-indigo-500 mb-0.5" />
                <span className="text-[6.5px] font-black text-neutral-400 uppercase tracking-wider leading-none mb-0.5">{distanceLabel}</span>
                <span className={`text-[9.5px] font-black font-mono leading-none ${theme === 'dark' ? 'text-neutral-200' : 'text-neutral-800'}`}>
              ...
              <button 
                ...
                className={`col-span-1 border rounded-xl flex flex-col items-center justify-center text-center transition-all p-1.5 active:scale-95 cursor-pointer pointer-events-auto ${theme === 'dark' ? 'bg-[#161622]/40 border-neutral-800/80 hover:bg-neutral-800 text-neutral-300' : 'bg-neutral-50 border-neutral-200/60 hover:bg-neutral-100 text-neutral-800'}`}
              >
                <span className="text-xs mb-0.5">🔗</span>
                <span className="text-[7.5px] font-black uppercase tracking-widest text-neutral-400">Share</span>
              </button>
              ...
              {onMessage ? (
                <button 
                  onClick={onMessage}
                  className={`col-span-1 border rounded-xl flex flex-col items-center justify-center text-center transition-all p-1.5 active:scale-95 cursor-pointer ${theme === 'dark' ? 'bg-indigo-950/20 border-indigo-900/40 text-indigo-400 hover:bg-indigo-950/40' : 'bg-indigo-50 border-indigo-100 hover:bg-indigo-100/80 text-indigo-600'}`}
                >
                  <MessageSquare className="w-3.5 h-3.5 text-indigo-500 mb-0.5" />
                  <span className="text-[7.5px] font-black uppercase tracking-widest">Chat</span>
                </button>
              ) : (
              ...
              {!isSpectator && (
                <button 
                  ...
                  className={`col-span-1 border rounded-xl flex flex-col items-center justify-center text-center transition-all p-1.5 active:scale-95 cursor-pointer ${theme === 'dark' ? 'bg-red-950/20 border-red-900/40 text-red-400 hover:bg-red-950/40' : 'bg-red-50 border-red-100 text-red-650 hover:bg-red-100'}`}
                >
                  <Shield className="w-3.5 h-3.5 text-red-500 mb-0.5" />
                  <span className="text-[7.5px] font-black uppercase tracking-widest">SOS</span>
                </button>
              )}
            </div>
            ...
            {!isSpectator && ride.status === 'on_trip' && (
              <button
                ...
                className={`w-full mt-2 h-9 border rounded-xl flex items-center justify-center gap-1.5 font-black tracking-wider text-[8.5px] uppercase transition-all active:scale-95 cursor-pointer pointer-events-auto ${theme === 'dark' ? 'bg-indigo-950/40 border-indigo-900/40 text-indigo-400 hover:bg-indigo-900/60' : 'bg-indigo-50 border-indigo-100 hover:bg-indigo-100 text-indigo-600'}`}
              >
            ...
            {!isSpectator && onCancel && (
              <button
                onClick={onCancel}
                className={`w-full mt-2.5 h-9 border rounded-xl flex items-center justify-center gap-1.5 font-black tracking-wider text-[8.5px] uppercase transition-all active:scale-95 cursor-pointer pointer-events-auto ${theme === 'dark' ? 'bg-red-950/10 border-red-900/30 text-red-400 hover:bg-red-950/30' : 'bg-red-50 border-red-100 hover:bg-red-100 text-red-650'}`}
              >
