...
export default function RiderHome({ onNavVisibilityChange, onProfileClick }: RiderHomeProps) {
  ...
  const getStartPin = (etaText: string) => {
    ...
    const cleanAddr = (addr: string) => {
      ...
      return addr.length > 22 ? addr.substring(0, 20) + "..." : addr;
    };
    ...
    const key = `rider-start-v3-${isDark}-${displayAddr}-${etaText || ''}`;
    ...
    const icon = L.divIcon({
      ...
      html: `
        <div class="relative flex flex-col items-center select-none" style="width: 120px;">
          <!-- Compact Callout Card Container -->
          <div class="relative flex flex-col items-start w-full transition-transform duration-150 transform hover:scale-105 filter drop-shadow-[0_4px_8px_rgba(0,0,0,0.25)]">
            <div class="w-full ${isDark ? 'bg-neutral-900 border-neutral-700 text-white' : 'bg-white border-neutral-200 text-neutral-900'} border rounded-lg p-1 px-1.5 shadow-md flex flex-col z-20">
              <div class="flex items-center justify-between gap-1 w-full">
                <div class="flex items-center gap-1 min-w-0">
                  <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0"></span>
                  <span class="text-[6px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-tight leading-none">MAHALI PA KUCHUKULIWA</span>
                </div>
                ${etaText ? `<span class="text-[6px] font-mono font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-1 rounded shrink-0 leading-tight">${etaText}</span>` : ''}
              </div>
              <span class="text-[8.5px] font-bold truncate leading-tight mt-0.5">${displayAddr}</span>
            </div>
          </div>

          <!-- Vertical Connecting Stem Line -->
          <div class="w-0.5 h-1.5 bg-emerald-500 shadow-sm z-10"></div>

          <!-- Ground Pin Dot Base -->
          <div class="relative flex items-center justify-center -mt-0.5 z-20">
            <div class="w-2.5 h-2.5 rounded-full bg-emerald-500 border border-white shadow-sm flex items-center justify-center">
  ...
  const getEndPin = (etaText: string) => {
    ...
    const key = `rider-end-v3-${isDark}-${displayAddr}-${etaText || ''}`;
    if (pinIconCacheMap[key]) {
      return pinIconCacheMap[key];
    }
    ...
    const icon = L.divIcon({
      ...
      html: `
        <div class="relative flex flex-col items-center select-none" style="width: 120px;">
            ...
            <div class="w-full ${isDark ? 'bg-neutral-900 border-neutral-700 text-white' : 'bg-white border-neutral-200 text-neutral-900'} border rounded-lg p-1 px-1.5 shadow-md flex flex-col z-20">
              <div class="flex items-center justify-between gap-1 w-full">
                <div class="flex items-center gap-1 min-w-0">
                  ...
                  <span class="text-[6px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-tight leading-none">HATIMA YAKO</span>
                </div>
                ${etaText ? `<span class="text-[6px] font-mono font-extrabold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-1 rounded shrink-0 leading-tight">${etaText}</span>` : ''}
              </div>
              <span class="text-[8.5px] font-bold truncate leading-tight mt-0.5">${displayAddr}</span>
            </div>
          </div>
          ...
          <!-- Vertical Connecting Stem Line -->
          <div class="w-0.5 h-1.5 bg-amber-500 shadow-sm z-10"></div>

          <!-- Ground Pin Dot Base -->
          <div class="relative flex items-center justify-center -mt-0.5 z-20">
            <div class="w-2.5 h-2.5 rounded-full bg-amber-500 border border-white shadow-sm flex items-center justify-center">
              <div class="w-0.5 h-0.5 rounded-full bg-white"></div>
            </div>
          </div>
        </div>
